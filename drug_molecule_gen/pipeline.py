...
def encoder_decoder_with_attn_mech():
    ...
    dec_embedding = Embedding(input_dim=len(config.BASE_VOCABULARY)+2,output_dim=(len(config.BASE_VOCABULARY)+2)//2,
                                input_length=config.MAX_OUTPUT_SEQUENCE_LEN,
                                name="decoder_embedding_layer")(dec_input)
    dec_lstm_layer = LSTM(units=(len(config.BASE_VOCABULARY)+2)//2,return_sequences=True,return_state=True,name="decoder_lstm_layer")
